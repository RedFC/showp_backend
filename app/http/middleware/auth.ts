...
var openApiKEY = fs.readFileSync("config/cert/openApiKey.pub", "utf8");
    ...
    openApiKeyValidate() {
        return(
            compose().use((req, res ,next)=>{
                const key = req.headers['open-api-key'];
                if(key==openApiKEY){
                    next()
                }else{
                    return res.send({success: false, msg:"The given Key was not matched"}); 
                }
